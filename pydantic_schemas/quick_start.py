...
def _is_pydantic_annotated_string(p, debug=False, indentation=""):
    if typing.get_origin(p) is typing.Annotated:
        ...
        if len(args) == 2 and args[0] is str:
            ...
            return True
        if debug:
            print(indentation, f"Is Annotated not string {p}")
    return False


def _create_default_class_from_annotation(p: Any, debug: bool = False, indentation: str = ""):
    if p is str:
        if debug:
...
def _create_default_from_list_of_args(
    args: List[Any], return_none_for_builtin_or_enum=True, debug=False, indentation=""
):
    """
    ...
    if len(typed_args):
        ...
        return _create_default_from_typing_annotation(chosen_type, debug=debug, indentation=indentation)
    elif len(pydantic_args):
        return make_skeleton(pydantic_args[0])
        ...
        if return_none_for_builtin_or_enum:
            return None
...
def _create_default_from_typing_annotation(p: Any, debug: bool = False, indentation=""):
    if debug:
    ...
    if isOptional:
        ...
        return _create_default_from_list_of_args(
            args, return_none_for_builtin_or_enum=True, debug=debug, indentation=indentation
        )
        ...
        if _is_pydantic_subclass(args[0]):
            ...
            return [_create_default(args[0], debug=debug, indentation=indentation + "  ")]
        ...
        return _create_default_from_list_of_args(
            args, return_none_for_builtin_or_enum=False, debug=debug, indentation=indentation
        )
    else:
        raise ValueError(f"Unknown typing {p}")


def _create_default(p: inspect.Parameter, debug: bool = False, indentation: str = ""):
    if hasattr(p, "annotation"):
        p = p.annotation
    if inspect.isclass(p) and not _is_typing_annotation(p):
        if debug:
            print(indentation, "CLASS")
        return _create_default_class_from_annotation(p, debug=debug, indentation=indentation)
    elif _is_typing_annotation(p):
        if debug:
            print(indentation, "TYPED")
        return _create_default_from_typing_annotation(p, debug=debug, indentation=indentation)
